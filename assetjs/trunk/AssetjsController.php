...
class AssetjsController extends PluginController
{
	/**
	 * Method: __construct
	 * =========================================================================
	...
	public function __construct()
	{
		if (defined('CMS_BACKEND'))
		{
			...
			$this->assignToLayout('sidebar', new View('../../plugins/assetjs/views/sidebar'));
		}
    }
	...
	{
        $this->display('assetjs/views/list', array(
            'assets' => Record::findAllFrom('Assetjs', '1=1 ORDER BY position')
        ));
    }
    ...
    public function add()
	{
        ...
        if (get_request_method() == 'POST')
        {
			...
			$asset = new Assetjs($data);

			// Save the asset
			if (!$asset->save())
			{
				Flash::set('error', __('Asset has not been added. Name must be unique!'));
				redirect(get_url('plugin/assetjs', 'add'));
			}
			else
			{
				Flash::set('success', __('Asset has been added!'));
				Observer::notify('assetjs_after_add', $asset);
			}

			// Save and quit or save and continue editing?
			if (isset($_POST['commit'])) redirect(get_url('plugin/assetjs'));
			else redirect(get_url('plugin/assetjs/edit/'.$asset->id));
        }
        else
        {
			...
			if (empty($asset)) $asset = new Assetjs();

			// Output the edit page
			$this->display('assetjs/views/edit', array
			(
				'action'  => 'add',
				'asset' => $asset
			));
	...
	public function edit($id)
	{
		...
		if (!$asset = Assetjs::findById($id))
		{
			...
			redirect(get_url('plugin/assetjs'));
		}
		...
		if (get_request_method() == 'POST')
		{
			...
			if ( ! $asset->save())
			{
				...
				redirect(get_url('plugin/assetjs/edit/'.$id));
			}
			else
			{
				Flash::set('success', __('Asset :name has been saved!', array(':name'=>$asset->name)));
				Observer::notify('assetjs_after_edit', $asset);
			}

			// Save and quit or Save and continue editing?
			if (isset($_POST['commit'])) redirect(get_url('plugin/assetjs'));
			else redirect(get_url('plugin/assetjs/edit/'.$id));
		}
		else
		{
			$this->display('assetjs/views/edit', array
    ...
    public function delete($id)
	{
		...
		if ($asset = Record::findByIdFrom('Assetjs', $id))
		{
			// Delete it
			if ($asset->delete())
			{
				Flash::set('success', __('Asset :name has been deleted!', array(':name'=>$asset->name)));
				Observer::notify('asset_after_delete', $asset);
			}
			else
			{
	...
	public function reorder()
	{
		...
		{
            $asset = Record::findByIdFrom('Assetjs', $asset_id);
	...
	public function output($name)
	{
		...
		// Output it
		header('Content-type: text/javascript;'); echo $output;

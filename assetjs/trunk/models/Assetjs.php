...
class Assetjs extends Record
{
    // The real Table Name
    const TABLE_NAME = 'assetjs';
    ...
    public function beforeSave()
    {
        ...
        $this->content_compressed = JSMin::minify($this->content);
        return true;
    }
    ...
    public static function find($args = null)
    {
        ...
        $tablename = self::tableNameFromClassName('Assetjs');
        ...
        if ($limit == 1)
        {
            return $stmt->fetchObject('Assetjs');
        }
        else
        {
            $objects = array();
            while ($object = $stmt->fetchObject('Assetjs'))
            {
                $objects[] = $object;
            }
            return $objects;
    ...
    public static function findById($id)
    {
        ...
        (
            'where' => self::tableNameFromClassName('Assetjs').'.id='.(int)$id,
            'limit' => 1
        ));
    }
}